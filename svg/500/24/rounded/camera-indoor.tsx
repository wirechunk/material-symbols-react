import type { SVGProps } from "react";
export const SvgCameraIndoor = (props: SVGProps<SVGSVGElement>) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    height={24}
    viewBox="0 -960 960 960"
    width={24}
    fill="currentcolor"
    {...props}
  >
    <path d="M242.87-111.87q-37.78 0-64.39-26.61t-26.61-64.39v-355.7q0-21.57 9.58-40.87 9.57-19.3 26.72-31.97L425.3-809.26q24.11-18.39 54.7-18.39 30.59 0 54.7 18.39l237.13 177.85q17.15 12.67 26.72 31.97 9.58 19.3 9.58 40.87v355.7q0 37.78-26.61 64.39t-64.39 26.61H242.87Zm0-91h474.26v-355.7L480-736.41 242.87-558.57v355.7ZM480-469.76ZM360-280h160q17 0 28.5-11.5T560-320v-40l52.43 27.72q9.51 5 18.54-.82 9.03-5.81 9.03-16.23v-101.34q0-10.42-9.03-16.23-9.03-5.82-18.54-.82L560-440v-40q0-17-11.5-28.5T520-520H360q-17 0-28.5 11.5T320-480v160q0 17 11.5 28.5T360-280Z" />
  </svg>
);
